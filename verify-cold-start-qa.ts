...
async function verifyColdStart() {
  ...
  const loggedOutRecs = await getColdStartRecommendations('sfw');
  
  console.log(`- Total recommendations: ${loggedOutRecs.length}`);
  console.log(`- Results from Trending: ${loggedOutRecs.filter(r => r.match_reasons.includes('Trending Now')).length}`);
  console.log(`- Results from Popular 30d: ${loggedOutRecs.filter(r => r.match_reasons.includes('Popular this Month')).length}`);
  console.log(`- Results from New & Noteworthy: ${loggedOutRecs.filter(r => r.match_reasons.includes('New & Noteworthy')).length}`);

  const hasNSFW = loggedOutRecs.some(r => ['erotica', 'pornographic'].includes(r.content_rating || ''));
  if (!hasNSFW) {
  ...
  const ids = loggedOutRecs.map(r => r.id);
  ...
  const t1Correct = tier1.every(r => r.match_reasons.includes('Trending Now'));
  const t2Correct = tier2.every(r => r.match_reasons.includes('Popular this Month'));
  const t3Correct = tier3.every(r => r.match_reasons.includes('New & Noteworthy'));
  ...
  loggedOutRecs.slice(0, 5).forEach((r, i) => {
