...
describe('Core Flow Integration Test', () => {
  ...
  test('Should process chapter ingestion and update feed correctly', async () => {
    ...
    const logicalChapter = await prisma.logicalChapter.findFirst({
      where: { series_id: testSeries.id, chapter_number: new Prisma.Decimal(1) }
    });
    expect(logicalChapter).toBeDefined();
    expect(logicalChapter?.chapter_title?.toLowerCase()).toBe('chapter one');
    ...
    expect(legacyChapter?.chapter_url).toBe(chapterData.chapterUrl);
  });
