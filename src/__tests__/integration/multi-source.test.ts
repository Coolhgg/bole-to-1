...
describe("Multi-Source Integration Logic", () => {
  ...
  it("should correctly prioritize sources based on user preference", async () => {
    ...
    const logicalChapter = await prisma.logicalChapter.create({
      data: {
        series_id: seriesId,
        chapter_number: 1.0,
        chapter_slug: "1",
      ...
      data: [
        {
          ...
          chapter_url: "https://mangadex.org/chapter/md-c1",
          language: "en",
        },
        {
          chapter_id: logicalChapter.id,
          series_source_id: source2.id,
          chapter_url: "https://mangasee123.com/chapter/ms-c1",
          language: "en",
        },
      ],
    })
    ...
    const priorityMap = new Map(priorities.map((p) => [p.source_name, p.priority]))
