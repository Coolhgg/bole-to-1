...
describe('Input Sanitization', () => {
  describe('sanitizeInput', () => {
    ...
    it('renames event handlers', () => {
      expect(sanitizeInput('test onclick=alert(1)')).toContain('data-sanitized-attr=')
      expect(sanitizeInput('onload=malicious')).toContain('data-sanitized-attr=')
    })
  ...
  describe('sanitizeFilterArray', () => {
    ...
    it('sanitizes each item', () => {
      expect(sanitizeFilterArray(['<script>test</script>clean', 'clean'])).toEqual(['clean', 'clean'])
    })
...
describe('Rate Limiting', () => {
  ...
  it('uses separate limits for different keys', async () => {
    for (let i = 0; i < 5; i++) {
      await checkRateLimit('key1', 5, 60000)
    }
    expect(await checkRateLimit('key1', 5, 60000)).toBe(false)
    expect(await checkRateLimit('key2', 5, 60000)).toBe(true)
  })

  it('resets after window expires', async () => {
    for (let i = 0; i < 3; i++) {
      await checkRateLimit('expire-test', 3, 50) // 50ms window
    }
    expect(await checkRateLimit('expire-test', 3, 50)).toBe(false)
    
    await new Promise(resolve => setTimeout(resolve, 60))
    
    expect(await checkRateLimit('expire-test', 3, 50)).toBe(true)
  })
})
