...
import { handleApiError, ApiError, ErrorCodes, validateContentType, validateJsonSize } from '@/lib/api-utils';
import { z } from 'zod';
          ...
          if (action.type === 'CHAPTER_READ') {
            ...
            if (logicalChapter) {
              const updateResult = await tx.$executeRaw`
                ...
                VALUES 
                  (gen_random_uuid(), ${user.id}::uuid, ${logicalChapter.id}::uuid, ${isRead}, ${targetTimestamp}::timestamptz, ${targetTimestamp}::timestamptz, ${sourceId}::uuid, ${action.deviceId}, NOW())
