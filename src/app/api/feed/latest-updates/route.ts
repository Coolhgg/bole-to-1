...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawResults = await prisma.$queryRawUnsafe(LATEST_UPDATES_SQL, limit + 1, offset) as LatestUpdateRow[];
    const countResult = await prisma.$queryRawUnsafe(LATEST_UPDATES_COUNT_SQL) as CountResult[];
