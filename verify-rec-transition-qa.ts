...
async function verifyTransitions() {
  ...
  const coldRecs = await getHybridRecommendations(userId, 0);
  console.log(`  Count: ${coldRecs.length}`);
  console.log(`  Sample Match Reasons: ${Array.from(new Set(coldRecs.map(r => r.match_reasons[0]))).join(', ')}`);
  
  // Test Warm State (5 interactions)
  console.log('\n- Testing WARM State (5 interactions):');
  const warmRecs = await getHybridRecommendations(userId, 5);
  console.log(`  Count: ${warmRecs.length}`);
  const hasPersonal = warmRecs.some(r => r.match_reasons[0].includes('interests') || r.match_reasons[0].includes('you'));
  ...
  const activeRecs = await getHybridRecommendations(userId, 15);
  console.log(`  Count: ${activeRecs.length}`);
  const hasPerfectMatch = activeRecs.some(r => r.match_reasons[0] === 'Perfect Match');
