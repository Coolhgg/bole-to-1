...
## Data Model (Review)
- `logical_chapters`: Container for a specific chapter number in a series.
- `chapter_sources`: Individual availability events (e.g., Chapter 10 on MangaDex, Chapter 10 on Bato.to).

## SQL Strategy
The `AVAILABILITY_FEED` query fetches the latest N records from `chapter_sources` and joins minimal metadata from `logical_chapters` and `series`.
