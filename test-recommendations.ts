...
async function runTest() {
  ...
  const recs = await getPersonalRecommendations(TEST_USER_ID);
  
  const romanceRecs = recs.filter(r => r.genres.includes('Romance'));
  ...
  const recsAfter = await getPersonalRecommendations(TEST_USER_ID);
  const isExcluded = !recsAfter.some(r => r.id === topRec.id);
  ...
  const recsAfterCompleted = await getPersonalRecommendations(TEST_USER_ID);
  const isCompletedExcluded = !recsAfterCompleted.some(r => r.id === nextRec.id);
